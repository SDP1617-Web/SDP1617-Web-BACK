INSERT INTO review (name, generation, team, content, created_at, updated_at)
SELECT '홍길동', 16, '테크 - 백엔드', '실제 서비스를 개발하며 스프링 부트를 깊게 익힐 수 있었습니다.', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM review WHERE name = '홍길동' AND generation = 16);

INSERT INTO review (name, generation, team, content, created_at, updated_at)
SELECT '김철수', 16, '테크 - 프론트엔드', '리액트로 실제 서비스를 만들어보는 경험이 정말 값졌습니다.', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM review WHERE name = '김철수' AND generation = 16);

INSERT INTO review (name, generation, team, content, created_at, updated_at)
SELECT '이영희', 17, '디자인', '개발자와 직접 소통하며 디자인 시스템을 구축하는 과정이 좋았습니다.', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM review WHERE name = '이영희' AND generation = 17);

INSERT INTO review (name, generation, team, content, created_at, updated_at)
SELECT '박영수', 17, '리서치', '논문 리뷰부터 실험까지 체계적으로 연구하는 방법을 배울 수 있었습니다.', NOW(), NOW()
